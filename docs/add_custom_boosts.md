...
```yaml
parameters:
    monsieurbiz.search.product.is_in_stock_scoring_boost: 0 # Weight (0 disables the boost)
...
The boost is enabled when the value of `monsieurbiz.search.product.is_in_stock_scoring_boost` is greater than 0, the request type is in `monsieurbiz.search.product.apply_is_in_stock_scoring_boost_on` and the stock filter is not enabled (`monsieurbiz.search.product.enable_stock_filter`).
...
- [Tag it with `monsieurbiz.search.request.product_function_score`](../dist/config/search/services.yaml)
